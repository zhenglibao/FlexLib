...
void FlexSetViewAttr(NSView* view,
                     NSString* attrName,
                     NSString* attrValue,
                     NSObject* owner)
{
...
-(NSView*)buildViewTree:(NSObject*)owner
    ...
    if(self.onPress.length>0){
        ...
        if(sel!=nil){
            if([owner respondsToSelector:sel]){
                NSClickGestureRecognizer *tap=[[NSClickGestureRecognizer alloc]initWithTarget:owner action:sel];
                                                ...
                                                NSColor* defaultColor)
{
    ...
    for (FlexAttr* attr in node.viewAttrs) {
        ...
        if( [attr.name isEqualToString:@"text"] ){
            ...
            NSFont* font = fontFromString(attr.value);
            [dict setObject:font forKey:NSFontAttributeName];
            
        }else if( [attr.name isEqualToString:@"fontSize"]){
            
            NSFont* font = [NSFont systemFontOfSize:[attr.value floatValue]];
            if(font!=nil){
                [dict setObject:font forKey:NSFontAttributeName];
            }
            ...
            NSColor* color = colorFromString(attr.value, owner);
            if( color!=nil){
...
//            NSImage* img = [NSImage imageNamed:attr.value inBundle:[owner bundleForImages] compatibleWithTraitCollection:nil];
//            attach.image = img ;
